...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
          ...
          @php
            $services = [["id"=>1,"name"=>"responsive", "price"=>400000],["id"=>2,"name"=>"responsive", "price"=>400000],["id"=>3,"name"=>"responsive", "price"=>400000],]
          @endphp

<div class="container mx-auto px-6 py-12">
    <h2 class="text-3xl font-bold text-center mb-8 text-gray-900 dark:text-gray-100">Order Your Website</h2>
    @if (session('success'))
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <!-- Order Form -->
        <div>
            <h3 class="text-3xl font-semibold mb-4 text-gray-900 dark:text-gray-100">Fill Out This Form</h3>
            <form action="/orders" method="POST">
              @if (session('success')) <div class="alert alert-success"> {{ session('success') }} </div> @endif
                @csrf
                <div class="mb-4">
                <label for="product" class="block text-gray-700 dark:text-gray-300"
                ...
                >
                  ...
                  <option value="{{ $p->id }}" {{ isset($product) && $p->id == $product->id ? 'selected' : '' }}>{{ $p->title }}</option>
                  @endforeach
                </select>
                @error("product")
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name" name="name" required :value="old('name')" class="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error("name")
                      <div class="text-red-600">
                        {{ $message }}
                      </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="phone_number" id="phone_number" required :value="old('phone_number')" name="phone_number" class="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error("name")
                      <div class="text-red-600">
                        {{ $message }}
                      </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="message" name="message" rows="4" class="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old("message") }}</textarea>
                </div>
                <button type="submit" class="w-full px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">Place Order</button>
            </form>
        </div>
        <!-- Contact Information -->
        <div>
